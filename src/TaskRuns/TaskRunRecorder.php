...
{
    public function start(Task $task, TaskOptions $options, RunManifest $manifest): ?Model
    {
        if (! $this->enabled()) {
            return null;
        }
        ...
        $run->recordRequest($manifest->toArray($this->logPrompt()));
    ...
    public function success(?Model $run, Usage $usage, ?RunManifest $manifest = null): void
    {
        ...
        if ($manifest instanceof RunManifest) {
            $run->recordRequest($manifest->toArray($this->logPrompt()));
        }
    ...
    private function logPrompt(): bool
    {
        return (bool) config('claude-tasks.log_prompt', true);
    }
}
